    ...
    class SearchController extends Controller {
        ...
        public static function getAdsList ($datasRequest, $_status = 'active') {
            ...
            if ($datasRequest) {
                ...
                $min_ad_price = (!empty($arrayGetAdsList['min_ad_price']) ? $arrayGetAdsList['min_ad_price'] : 0);
                $max_ad_price = (!empty($arrayGetAdsList['$max_ad_price']) ? $arrayGetAdsList['$max_ad_price'] : 0);
            ...
            $countrycontractsIds = [];
            if(session()->has('countrycontractsIds')) {
                $countrycontractsIds = session()->get('countrycontractsIds');
                ...
                session()->put('countrycontractsIds', $countrycontractsIds);
            }

            $AdCountryCodeCount = Gateway::where('status', 'active')->where('ad_country_code', $country_code)->where('ad_price', '>=', $min_ad_price)->count();
            $AdCountryIdCount = Gateway::where('status', 'active')->where('countries_id', $countries_id)->where('ad_price', '>=', $min_ad_price)->count();

            $ads_list = ['data'=>''];

            if(!empty($countrycontractsIds)) {
                // filter by country only on homepage for recent ads
                if ($AdCountryCodeCount > 0 && app('request')->route()->getName() == 'homepage') {
                    $WhereRaw .= 'AND (' . 'countrycontracts_id IN (' . implode(',', $countrycontractsIds) . ') AND ad_country_code = "' . $currentCountryCode . '")';

                } else if($AdCountryIdCount > 0 && app('request')->route()->getName() == 'homepage') {
                   //$WhereRaw .= 'AND ' . 'countrycontracts_id IN (' . implode(',', $countrycontractsIds) . ') AND ' . 'countries_id = "' . $countries_id . '" ';
                ...
                // filter by country only on homepage for recent ads
                if ($AdCountryCodeCount > 0 && app('request')->route()->getName() == 'homepage') {
                    $WhereRaw .= 'AND ' . 'ad_country_code = "' . $currentCountryCode . '" ';
                } else if($AdCountryIdCount > 0 && app('request')->route()->getName() == 'homepage') {
                    $WhereRaw .= 'AND ' . 'countries_id = "' . $countries_id . '" ';
                }
            }
            $total_new_boats = Gateway::select('id')
                ->whereRaw($WhereRaw)
                ->where('sell_type', '=', 'new')
                ->count();
            $total_used_boats = Gateway::select('id')
                ->whereRaw($WhereRaw)
                ->where('sell_type', '=', 'used')
                ->count();
            $ads_list = Gateway::select(
                ...
                ->paginate($max_query);
                //->get();
                $ads_list['total_used_boats'] = $total_used_boats;
                $ads_list['total_new_boats'] = $total_new_boats;
        ...
        public static function getTotal ($what='status', $value='active', $WhereRaw= '') {
            ...
            } else if (!empty($WhereRaw)) {
                //$total = Gateway::where('status', 'active')->where($what, $value)->whereRaw($WhereRaw)->count();
            }
            if(empty($total)) {
                $total = 0;
            }
