...
?>
    <div class="clearfix gallery">
        <h3>{!! trans('sell.gallery_photos') !!}</h3>

        @include('theme.partials.upload-form', ['ad_id'=>$ad_id, 'host'=>config('youboat.' . $country_code . '.website_youboat_url'), 'root'=>config('youboat.' . $country_code . '.root'), 'custom_dir'=>'/assets/' . $targetDir . '/', 'ad_photos'=>$ad_photos, 'targetDir'=>$targetDir])
