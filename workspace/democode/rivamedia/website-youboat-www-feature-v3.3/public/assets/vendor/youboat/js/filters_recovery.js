$(document).ready(function () {
    ...
    var recovery_description        = $RecoveryDescription.length && $RecoveryDescription.val() ? $RecoveryDescription.val() : '';
    ...
    if ($CollapseRecovery.length) {
        if (recovery_adstypes_id > 0|| recovery_category_id > 0 || recovery_subcategory_id > 0 ||
            recovery_manufacturers_id > 0 || recovery_models_id > 0 || recovery_year_built > 0 ||
            recovery_budget > 0 ||
            recovery_description != '') {
            jQuery('#btn_recovery').trigger('click');
        }

        $CollapseRecovery.on('shown.bs.collapse', function () {
        ...
        $CollapseRecovery.on('hide.bs.collapse', function () {
            ...
            $RecoveryAdstypesId.val('');
            $RecoveryCategoriesIds.val('');
            $RecoverySubcategoriesIds.val('');
            $RecoveryManufacturersId.val('');
            $RecoveryModelsId.val('');
            $RecoveryYearBuilt.val('');
            $RecoveryBudget.val('');
            $RecoveryDescription.val('');
    ...
    if ($RecoveryAdstypesId.length) {
        ...
        $RecoveryAdstypesId.on('change', function (event) {
            ...
            var recovery_category_id = $RecoveryCategoriesIds.val() ? $RecoveryCategoriesIds.val() : '';
            var recovery_subcategory_id = $RecoverySubcategoriesIds.val() ? $RecoverySubcategoriesIds.val() : '';
    ...
    if ($RecoveryCategoriesIds.length) {
        ...
        $RecoveryCategoriesIds.on('change', function (event) {
            ...
            var recovery_adstypes_id = $RecoveryAdstypesId.val() ? $RecoveryAdstypesId.val() : '';
            var recovery_subcategory_id = $RecoverySubcategoriesIds.val() ? $RecoverySubcategoriesIds.val() : '';
    ...
    if ($RecoveryManufacturersId.length) {
        dataAjaxUrl = '';
        recovery_adstypes_id = $RecoveryAdstypesId.val() ? $RecoveryAdstypesId.val() : '';
        recovery_category_id = $RecoveryCategoriesIds.val() ? $RecoveryCategoriesIds.val() : '';
        recovery_subcategory_id = $RecoverySubcategoriesIds.val() ? $RecoverySubcategoriesIds.val() : '';
