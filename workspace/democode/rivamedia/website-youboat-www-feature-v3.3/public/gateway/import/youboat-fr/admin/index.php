<?php
	...
	switch(RUN_TYPE)
	{
		case "test":
			...
			define('PATH_IMG', '/htdocs/youboat-www/youboat-www_boatgest/public/assets/photos/uk/');
			define('ID_AUTOMAT', 1);
		break;
		case "debug":
	...
	*/
	...
	{
		$sql = "DELETE FROM gateway_error WHERE id=".$_POST['erreur'];
		mysql_query($sql);
	}
...
<html>
	...
	<body>
		...
		<table class="table" width="100%">
			...
			<?php
				$class='tr1';
				$sql = "SELECT id, gateway_id, manufacturers, models, date FROM gateway_error ORDER BY id DESC LIMIT 50";
								...
								<td>';
									if(!passerelle_blacklist($data['gateway_id'], 'manufacturers', $data['manufacturers']))
									{
										echo '<form method="post" action="'.$currenturl.'">
												...
												<input type="text" name="marque" value="'.$data['manufacturers'].'" /><br /><input type="submit" value="Créer" />
											</form>';
										echo '<form method="post" action="'.$currenturl.'">
								...
								if(!empty($data['models']))
								{
									if(!passerelle_blacklist($data['gateway_id'], 'models', $data['models']))
									{
										echo '<form method="post" action="'.$currenturl.'">
												...
												<input type="text" name="modele" value="'.$data['models'].'" /><br /><input type="submit" value="Créer" />
											</form>';
										echo '<form method="post" action="'.$currenturl.'">
