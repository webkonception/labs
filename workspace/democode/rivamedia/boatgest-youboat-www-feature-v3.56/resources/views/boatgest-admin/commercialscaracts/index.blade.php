...
@section('content')
    ...
    <p>{!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.commercialscaracts.create', '<i class="fa fa-plus fa-fw"></i>Add new', [], array('class' => 'btn btn-success'))) !!}</p>

@if ($commercialscaracts->count())
    <div class="panel panel-success">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-striped table-hover datatable">
                ...
                <tbody>
                @foreach ($commercialscaracts as $row)
                    <?php
                        ...
                        $name .= !empty($row->user_id) && !empty($commercialsusernames[$row->user_id]) ? '<br><em>[' . $commercialsusernames[$row->user_id] . ']</em>' : '';
                    ...
                    <tr {!! $strike !!}>
                        ...
                        <td class="{!! $css_status !!}">
                            {!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.commercialscaracts.edit', '<i class="fa fa-pencil fa-fw"></i>Edit', [$row->id], ['class' => 'btn btn-block btn-xs btn-primary'])) !!}
                            @if($isAdmin)
                            <br>
                            {!! Form::open(array('class' => '', 'method' => 'DELETE', 'onsubmit' => 'return confirm(\'Confirm deletion\');',  'route' => array(config('quickadmin.route') . '.commercialscaracts.destroy', $row->id))) !!}
                        ...
                        <td class="{!! $css_status !!}">{!! $row->phone_1 !!}</td>
            ...
            {!! Form::open(['route' => config('quickadmin.route') . '.commercialscaracts.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
