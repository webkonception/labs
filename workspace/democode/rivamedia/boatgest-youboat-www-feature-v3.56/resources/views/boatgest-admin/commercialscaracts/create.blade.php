...
@section('content')
    ...
    {!! Form::open(array('route' => config('quickadmin.route') . '.commercialscaracts.store', 'id' => 'form-with-validation', 'role'=>'form', 'role'=>'form', 'class' => 'form-horizontal')) !!}
    <section class="row well well-sm well-white">
        <div class="col-sm-6">
            ...
            <div class="form-group {!! $css_state !!}">
                {!! Form::label('user_id', $label_txt, ['class'=>'col-xs-3 col-sm-5 control-label']) !!}
                <div class="col-xs-{{ (count($commercialsusernames) > 1 && $isAdmin) ? 6 : 9 }} col-sm-{{ (count($commercialsusernames) > 1 && $isAdmin) ? 5 : 7 }}">
                    <div class="input-group">
                        @if (!empty($username))
                            {!! Form::text('username', $username, $attributes) !!}
                            {!! Form::hidden('user_id', $user_id) !!}
                        @elseif (count($commercialsusernames) < 1 && $isAdmin)
                        ...
                        @else
                            {!! Form::select('user_id', $commercialsusernames, $user_id, $attributes) !!}
                        @endif
                        <span class="input-group-addon"><span class="fa fa-life-ring"></span></span>
                    </div>
                </div>
                @if (count($commercialsusernames) > 1 && $isAdmin)
    ...
    @if($isAdmin)
    <?php
        /*$username = empty($username) && !empty($commercialsusernames[$user_id]) ? $commercialsusernames[$user_id] : '';
        $rewrite_url = old('rewrite_url', !empty($username) ? 'https://commercials.youboat.com/' . str_slug($username) : '');
    ...
    {{--<section class="row well well-sm well-white">
        <div class="col-sm-12">
            <div class="form-group {!! $css_state !!}">
                ...
                <div class="col-xs-9 col-sm-8">
                    {!! Form::text('rewrite_url', $rewrite_url, $attributes) !!}
                </div>
            </div>
        </div>
    ...
    <div class="form-group">
        <div class="col-sm-12 text-center">
            ...
            {!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.commercialscaracts.index', '<i class="fa fa-mail-reply fa-fw"></i>' . ucfirst(trans('navigation.back')), [], array('class' => 'btn btn-default pull-right'))) !!}
        </div>
    </div>
