...
@section('content')
    ...
    <p>{!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.dealerscaracts.create', '<i class="fa fa-plus fa-fw"></i>Add new', [], array('class' => 'btn btn-success'))) !!}</p>

    @if ($dealerscaracts->count())
    <div class="panel panel-success">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-striped table-hover datatable">
                ...
                @foreach ($dealerscaracts as $row)
                    <?php
                        ...
                        $name .= !empty($row->firstname) && !empty($row->name) ? '<br><b>' . ucfirst(mb_strtolower($row->firstname)) . '</b>' : '';
                        $name .= !empty($row->name) ? (empty($name) ? '' : ' <b>') . mb_strtoupper($row->name) . '</b>' : '';
                        $name .= !empty($row->user_id) && !empty($dealersusernames[$row->user_id]) ? '<br><em>[' . $dealersusernames[$row->user_id] . ']</em>' : '';
                    ...
                    <tr {!! $strike !!}>
                        ...
                        <td class="{!! $css_status !!}">
                            {!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.dealerscaracts.edit', '<i class="fa fa-pencil fa-fw"></i>Edit', [$row->id], ['class' => 'btn btn-block btn-xs btn-primary'])) !!}
                            @if($isAdmin)
                            <br>
                            {!! Form::open(array('class' => '', 'method' => 'DELETE', 'onsubmit' => 'return confirm(\'Confirm deletion\');',  'route' => array(config('quickadmin.route') . '.dealerscaracts.destroy', $row->id))) !!}
                        ...
                        <td class="{!! $css_status !!}">
                            ...
                            {{--<div class="form-group">
                                {!! Form::open(array('class' => '', 'method' => 'POST',  'route' => array('ajax_update', ''))) !!}
                                {!! Form::hidden('where', 'dealerscaracts') !!}
                                {!! Form::hidden('id', $row->id) !!}
                                {!! Form::hidden('what', 'status') !!}
                                {!! Form::select('value', $status, $row->status, $attributes) !!}
                                {!! Form::close() !!}
                            </div>--}}
                        </td>
            ...
            {!! Form::open(['route' => config('quickadmin.route') . '.dealerscaracts.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
