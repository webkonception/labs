...
@section('content')
    ...
    <p>{!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.customerscaracts.create', '<i class="fa fa-plus fa-fw"></i>Add new', [], array('class' => 'btn btn-success'))) !!}</p>

@if ($customerscaracts->count())
    <div class="panel panel-success">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-striped table-hover datatable">
                ...
                <tbody>
                @foreach ($customerscaracts as $row)
                    <?php
                        ...
                        $name .= !empty($row->user_id) && !empty($customersusernames[$row->user_id]) ? '<br><em>[' . $customersusernames[$row->user_id] . ']</em>' : '';
                    ...
                    <tr {!! $strike !!}>
                        ...
                        <td class="{!! $css_status !!}">
                            {!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.customerscaracts.edit', '<i class="fa fa-pencil fa-fw"></i>Edit', [$row->id], ['class' => 'btn btn-block btn-xs btn-primary'])) !!}
                            @if($isAdmin)
                            <br>
                            {!! Form::open(array('class' => '', 'method' => 'DELETE', 'onsubmit' => 'return confirm(\'Confirm deletion\');',  'route' => array(config('quickadmin.route') . '.customerscaracts.destroy', $row->id))) !!}
            ...
            {!! Form::open(['route' => config('quickadmin.route') . '.customerscaracts.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
