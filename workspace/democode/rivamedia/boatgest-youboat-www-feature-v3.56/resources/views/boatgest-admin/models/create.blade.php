...
@section('content')
    ...
    {!! Form::open(array('route' => config('quickadmin.route') . '.models.store', 'id' => 'form-with-validation', 'role'=>'form', 'class' => 'form-horizontal')) !!}
        <div class="row">
            <div class="col-sm-6">
                <div class="form-group">
                    {!! Form::label('manufacturers_id', trans('manufacturers.manufacturers') . '*', ['class'=>'col-xs-12 col-sm-4 control-label']) !!}
                    <div class="{{ (count($manufacturers) > 1 && $isAdmin) ? 'col-xs-9' : '' }} col-sm-{{ (count($manufacturers) > 1 && $isAdmin) ? 6 : 8 }}">
                    ...
                    @if (count($manufacturers) > 1 && $isAdmin)
                        <div class="col-xs-3 col-sm-2">
                            {!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.manufacturers.create', '<i class="fa fa-plus fa-fw"></i>' . trans('navigation.add'), [], ['class'=>'blank btn btn-sm btn-success'])) !!}
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-sm-6">
                <div class="form-group">
                    {!! Form::label('name', trans('models.model') . ' Name*', ['class'=>'col-xs-4 col-sm-4 control-label']) !!}
        ...
        <div class="form-group">
            <div class="col-sm-12 text-center">
                ...
                {!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.models.index', '<i class="fa fa-mail-reply fa-fw"></i>' . ucfirst(trans('navigation.back')), [], array('class' => 'btn btn-default pull-right'))) !!}
            </div>
        </div>
