...
@section('content')
    ...
    {!! Form::open(array('route' => config('quickadmin.route') . '.privatescaracts.store', 'id' => 'form-with-validation-create', 'role'=>'form', 'files'=>true, 'class' => 'form-horizontal')) !!}
    <section class="row well well-sm well-white">
        <div class="col-sm-6">
            ...
            <div class="form-group {!! $css_state !!}">
                ...
                <div class="col-xs-{{ (count($privatesusernames) > 1 && $isAdmin) ? 6 : 9 }} col-sm-{{ (count($privatesusernames) > 1 && $isAdmin) ? 6 : 8 }}">
                    <div class="input-group">
                        @if (!empty($username))
                            {!! Form::text('username', $username, $attributes) !!}
                            {!! Form::hidden('user_id', $user_id) !!}
                        @elseif (count($privatesusernames) < 1 && $isAdmin)
                        ...
                        @else
                            {!! Form::select('user_id', $privatesusernames, $user_id, $attributes) !!}
                        @endif
                        <span class="input-group-addon"><span class="fa fa-anchor"></span></span>
                    </div>
                </div>
                @if (count($privatesusernames) > 1 && $isAdmin)
    ...
    <section class="row well well-sm well-white">
        ...
        <div class="col-sm-6">
            ...
            if (!empty($photo)) {
                $css_state = 'has-success';
                $ad_img_params = ['ad_id'=>'private', 'ad_title'=>$rewrite_url, 'image_name'=>'logo'];
                $url_image_ext = url_image_ext('', $photo, 'photos/privates/' . $country_code . '/', $ad_img_params);
            }
            if ($errors->has('name')) {
                $css_state = 'has-error';
            }
    ...
    <div class="form-group">
        <div class="col-sm-12 text-center">
            ...
            {!! htmlspecialchars_decode(link_to_route(config('quickadmin.route') . '.privatescaracts.index', '<i class="fa fa-mail-reply fa-fw"></i>' . ucfirst(trans('navigation.back')), [], array('class' => 'btn btn-default pull-right'))) !!}
        </div>
    </div>
