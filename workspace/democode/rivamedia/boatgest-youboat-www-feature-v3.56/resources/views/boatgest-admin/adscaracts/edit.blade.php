<?php
    ...
    */
    ...
    if(isset($chartTotalVisitorsAndPageViewsByPath) && !empty($chartTotalVisitorsAndPageViewsByPath)) {
    ...
    if($_TotalVisitorsAndPageViewsByPath) {
        ...
        $statistics .=
        '<div id="TotalVisitorsAndPageViewsByPath" class="portlet box purple">' . "\n" .
        '    <div class="portlet-title">' . "\n" .
    ...
    if(!preg_match('/\/' . $ad_id . '$/', $EffectiveUrl)) {
        $EffectiveUrl = '#' . $ad_id;
    }
?>
...
@section('content')
    ...
    <div class="row">
        <div class="col-xs-4 col-sm-2">
            {!! Form::open(array('id' => 'form_delete_ad', 'class' => '', 'method' => 'DELETE', 'onsubmit' => 'return confirm(\'Confirm deletion\');',  'route' => array(config('quickadmin.route') . '.adscaracts.destroy', $ad_id))) !!}
            {{--{!! Form::button('<i class="fa fa-trash-o fa-fw"></i>' . trans('navigation.delete') . '?', array('type' => 'submit', 'class' => 'btn btn-block btn-md btn-danger btn-exception')) !!}--}}
            {!! Form::close() !!}
        </div>
    </div>
    <hr>
    ...
    <section class="row well well-sm well-warning text-center">
        <div class="col-sm-12 col-md-4 text-primary text-right">
            <?php
            ...
            ?>
            <div class="form-group {!! $css_state !!}">
                {!! Form::label($input_name, $label_txt, ['class'=>'col-xs-5 control-label strong text-primary']) !!}
                <div class="col-xs-7">
                    {!! Form::select($input_name, $select_values, $input_value, $attributes) !!}
                </div>
            </div>
        </div>
        <div class="col-sm-12 col-md-4 text-primary lead">
            <span class="strong badge badge-primary">Created at</span>
            {!! $AdsCaracts->created_at->format('Y-m-d H:i:s') !!}
        </div>
        <div class="col-sm-12 col-md-4 text-success lead">
        ...
        <div class="col-sm-12 text-primary text-right">
            <hr>
            {!! Form::button('<i class="fa fa-trash-o fa-fw"></i>' . trans('navigation.delete') . '?', array('type' => 'button', 'class' => 'btn btn-md btn-danger btn-exception', 'id' => 'btn_delete_ad')) !!}
        </div>
    </section>
    ...
    <section class="row well well-sm well-success">
        ...
        <div class="col-sm-12">
            ...
            $input_value = empty($input_value) && !empty($ad_id) ? $EffectiveUrl : old($input_name);
            ...
            <div class="form-group {!! $css_state !!}">
                ...
                <div class="col-xs-12 col-sm-10">
                    <legend>
                        ...
                        <a href="{!! $input_value !!}" class="blank" title="{!! $input_value !!}">{!! $input_value !!}<i class="fa fa-external-link-square fa-fw"></i></a>
                        @else
                        {!! $input_value !!}
                        @endif
                    </legend>
    ...
    <ul class="nav nav-tabs" role="tablist">
        ...
        @if(!empty($statistics))
        <li role="presentation"><a class="bg-purple" href="#statistics" aria-controls="statistics" role="tab" data-toggle="tab">{!! ucfirst(trans('navigation.statistics')) !!} <i class="fa fa-line-chart fa-fw"></i></a></li>
        @endif
    </ul>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
            var $BtnDeleteAd = $('#btn_delete_ad');
            var $FormDeleteAd = $('#form_delete_ad');
            $BtnDeleteAd.on('click', function() {
                $FormDeleteAd.trigger('submit');
            });
        });
    </script>
    <script>
