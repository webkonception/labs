<?php
	...
	$tmarque = array();
	$sql = "SELECT id, rewrite FROM manufacturers";
	$req = mysql_query($sql);
	while($data = mysql_fetch_assoc($req)){ $tmarque[] = $data['rewrite'].'|'.$data['manufacturers_id']; }
	...
	function match_modele($modele, $manufacturers_id)
	{
		...
		for($i=count($tabtext);$i>0;$i--)
		{
			...
			$sql = "SELECT id, rewrite FROM models WHERE manufacturers_id=".$manufacturers_id." AND (rewrite='".strip_specialchar($matchtext)."' OR rewrite='".strip_specialchar($matchtextspace)."') ORDER BY id LIMIT 1";
			$req = mysql_query($sql);
			$row = mysql_fetch_row($req);
			if(!empty($row[0]))
			{
		...
		if(!empty($returnmodele) && !empty($returnmodeletext))
		{
			...
			{
				$sql = "SELECT id FROM models WHERE manufacturers_id=".$manufacturers_id." AND (rewrite='".$returnmodeletext.'-'.strip_specialchar($word)."' OR rewrite LIKE '".$returnmodeletext.'-'.strip_specialchar($word)."-%')";
				$req = mysql_query($sql);
				$row = mysql_fetch_row($req);
				if(!empty($row[0]))
				{
